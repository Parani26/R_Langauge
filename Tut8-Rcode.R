...
classCounts <- classCounts/rowSums(classCounts); classCounts # survived in rows

genderCounts <- table(titanic[,c("Survived", "Sex")]); genderCounts
genderCounts <- genderCounts/rowSums(genderCounts) ; genderCounts # survived in rows

ageCounts <- table(titanic[,c("Survived", "Age")]); ageCounts
ageCounts <- ageCounts/rowSums(ageCounts); ageCounts # survived in rows
...
score <- pred.M1[, 2] #only takes yes 
# new_titanic_sur <- titanic$sur == 1 optional line as score i already formatted to take yes only
...
pred.M2= as.numeric(paste(pred.M2)) # changing format to numeric
