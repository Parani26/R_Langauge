...
summary(as.factor(churn$Churned)) #instead of using table
...
for(i in 1:length(cp)){
	...
	for (j in 1:n_folds) {
		...
		new.data=data.frame(banktrain[test,c(1:8)]) # chcek if response there if not there remove it nefore inputing test or train data
